...
{
    private readonly Func<int, CancellationToken, Task<HttpResponseMessage>> _reply;

    private FakeHandler(Func<int, CancellationToken, Task<HttpResponseMessage>> reply) => _reply = reply;

    public List<(HttpRequestMessage Request, string Body)> Requests { get; } = [];

    public static FakeHandler Json(string body, HttpStatusCode status = HttpStatusCode.OK) =>
        new((_, _) => Task.FromResult(Response(body, status)));

    public static FakeHandler Sequence(params string[] bodies) =>
        new((index, _) => index < bodies.Length
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        return await _reply(index, cancellationToken);
    }
